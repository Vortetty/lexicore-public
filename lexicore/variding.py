...
def decoder(input):
    ...
    if int(tableID) < 32:
        ...
        for i in range(len(text)):
            preout = ""
            print("Chunk " + str(i+1) + ": " + text[i] + " (" + str(lists[str(tableID)].index(text[i])) + ")" + " → " + str("{0:b}".format(lists[str(tableID)].index(text[i]), 2)))
            preout = str("{0:b}".format(lists[str(tableID)].index(text[i]), 2))
        ...
        for i in range(len(text)):
            preout = ""
            print("Chunk " + str(i+1) + ": " + str(text[i]) + " (" + str(lists["32+d"](text[i], n1, shiftAmmnt) + add) + ") → " + str("{0:b}".format(lists["32+d"](text[i], n1, shiftAmmnt) + add, 2)))
            preout = str("{0:b}".format(lists["32+d"](text[i], n1, shiftAmmnt) + add,2))
            while len(preout) < int(tableID):
                preout = "0" + preout
            output += preout
        output = frombits(output)
    ...
    prepremetadata = ""
    for i in range(len(metadata)):
        premetadata = str("{0:b}".format(metaTable.index(metadata[i]), 2))
